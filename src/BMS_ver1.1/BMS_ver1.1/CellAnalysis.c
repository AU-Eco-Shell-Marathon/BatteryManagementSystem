...
unsigned char increase_measurement_rate(void)
{
	...
	else if(((Tbatt[1] > (Tcharge_max-5)) || (Tbatt[1] < (Tcharge_min+5))) && (Icharge>Idischarge))
	warning_repport = 3;
...
unsigned char Safety_check (void)
{
	...
	//As no charger sense is used the new temperature threshold is limited by charger threshold
	else if(Icharge>Idischarge)
	{
...
{
	unsigned int Vcell_IRcompensated[18];
	...
	if((maxV-minV) > 10)
	{
		...
		avr = sum/Cell_count;
		for(int i=1;i<=6;i++)//i<=Cell_count;i++)
		{
		...
		balance_cell(1,balance);
		uart_mini_printf("\r\nDevice1 balanceVar %u\r\n",balance);
		//Reset Byte before analyzing for next device
		balance = 0;
		for(int i=7;i<=12;i++)//i<=Cell_count;i++)
		{			
			if (Vcell_IRcompensated[i]>avr)
			{
				balance |= (1<<(i-7));
		...
		balance_cell(2,balance);
		uart_mini_printf("\r\nDevice2 balanceVar %u\r\n",balance);
	}	
}
