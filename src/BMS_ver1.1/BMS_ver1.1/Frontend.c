...
void init_frontend(void)
{
		...
		send_Data_and_CRC(0x02,0x30,0b00000101);
		_delay_ms(1);
...
void collect_ADC_values(void)
{
	...
	for (int x=1;x<=number_of_BQ76PL536a_in_stack;x++)
	{
		...
		CRC_error = receive_Data_and_CRC((x),0x00,1);
		...
		if (CRC_error == 0 && (Rec_buffer[0] & 0b00000001))
		{
			...
			CRC_error = receive_Data_and_CRC((x),0x00,18);
			//uart_mini_printf ("\r\n  CRCerror = %u on ic%u\r\n",CRC_error,(x));
			...
			if(CRC_error == 0)
			{
				...
				for(int i=0;i<6;i++)
				{
					...
					//uart_mini_printf ("\r\nCelln%u converted to  %umV on ic%u\r\n",n,Vcell[n],(x));
				...
				if(x == 1)
				{
					...
					{
						Idischarge = (long)(Idischarge - Idis_offset)*35/10;
					}
					...
					{
						Icharge = (long)(Icharge - Ich_offset)*1480/1000;
					}
					//Print for test
					//uart_mini_printf ("\r\n Icharge = %u\r\n",Icharge);
				...
				//Include code below to enable conversion of temperature sensors on extension module, furthermore enable in ADC config 
...
void write_protected_registers(void)
{
	...
	send_Data_and_CRC(0x01,0x47,0x03);
		...
		//write protected registers of ic 2
		//Setup ADC mux for GPIO and bandgab reference, conversion time 6us
		send_Data_and_CRC(0x02,0x3a,0x35);
		send_Data_and_CRC(0x02,0x40,0b01000000);
		
		//Overwrite COV register as fault triggers at 4100mV even though
		//it is programmed for 4200mV (set to (theoretically) 4250mV)
		send_Data_and_CRC(0x02,0x3a,0x35);
		send_Data_and_CRC(0x02,0x42,0x2e);
		send_Data_and_CRC(0x02,0x3a,0x35);
		send_Data_and_CRC(0x02,0x43,0x8a);
		
		//Rewrite CUV as this funbction is also used at EPROM errors(2900mV)
		send_Data_and_CRC(0x02,0x3a,0x35);
		send_Data_and_CRC(0x02,0x44,0x15);
		send_Data_and_CRC(0x02,0x3a,0x35);
		send_Data_and_CRC(0x02,0x45,0x8f);
		
		//Rewrite overtemperature as this function is also used at EPROM errors
		send_Data_and_CRC(0x02,0x3a,0x35);
		//60 degrees max for TS2, 19A for TS1
		send_Data_and_CRC(0x02,0x46,0x5B);
		send_Data_and_CRC(0x02,0x3a,0x35);
		send_Data_and_CRC(0x02,0x47,0x03);
}

void handle_Alerts_and_Faults(void)
{
